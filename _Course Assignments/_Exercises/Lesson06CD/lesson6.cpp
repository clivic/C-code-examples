	...
	while(!done)									// Loop That Runs While done=FALSE
	{
		...
		else										// If There Are No Messages
		{
			...
			else									// Not Time To Quit, Update Screen
			{
				...
				if (keys['W'])
				{
					xpos -= (float)sin(heading*PI_OVER_180) * 0.01f;			// 沿游戏者所在的X平面移动
					zpos -= (float)cos(heading*PI_OVER_180) * 0.01f;			// 沿游戏者所在的Z平面移动
					//zpos -= 0.001f;
				...
				if (keys['A'])
				{
					xpos -= (float)cos(heading*PI_OVER_180) * 0.01f;			// 沿游戏者所在的X平面移动
					zpos += (float)sin(heading*PI_OVER_180) * 0.01f;			// 沿游戏者所在的Z平面移动
					//xpos -= 0.001f;
					if (walkbiasangle >= 359.0f)					// 如果walkbiasangle大于359度
					{
						walkbiasangle = 0.0f;					// 将 walkbiasangle 设为0
