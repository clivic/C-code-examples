...
int String::Find(const String &target)
//	Post:	The function returns a position(0 ~ Strlen(*this)) of the 
//			head character of the first occurrence of the string target 
//			in the string, otherwise it returns -1 if the string target 
//			is not present in text.
{
	return Strstr(*this, target);
}
//safety guards
String::~String()
{
...
String* Strncpy(String &to, const String &from, int n)
//	Post:	The function copies the first n characters in String from 
//			to String to and returns the address of String to.
{
	const char *cFrom = from.c_str();
	char *copy = new char[n + 1];
	Strncpy(copy, cFrom, n);
	to = copy;
	copy = NULL;
	return &to;
}
