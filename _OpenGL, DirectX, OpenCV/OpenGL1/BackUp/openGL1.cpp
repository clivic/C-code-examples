...
// 描述:	正常销毁窗口
{
	...
	// 能否注销类?
	if (!UnregisterClass(L"OpenG", hInstance))
	{
		MessageBox(NULL, L"Fail to unregister class \"OpenG\".", L"Shutdown Error", MB_OK | MB_ICONINFORMATION);
...
//	描述:	Windows程序的入口。将会调用窗口创建例程，处理窗口消息，并监视人机交互
{
	...
	// 提示用户选择运行模式
	if (MessageBox(NULL, L"Welcome!\n Would you like to run in FullScreen?", L"Welcome", MB_YESNO | MB_ICONQUESTION) == IDNO)
	{
	...
	{
		if (GetMessage(&msg, NULL, 0, 0))		// 有消息在等待吗?
		{
