...
// 描述:	从这里开始进行所有的绘制
{
	...
	// 先設定旋轉
	glRotatef(rtri, 1.0f, 1.0f, 0.0f);					// 绕Y轴旋转三角形
	...
	glTranslatef(1.5f, 0.0f, -7.0f);					// 重新從原點開始放好正方形中心之位置,在三角形中心下面2.0f處,並更加移入屏幕
	glRotatef(rquad, 1.0f, 1.0f, 1.0f);					// 绕X=Y=Z旋转四边形

	// 绘制正方形
	glBegin(GL_QUADS);	
	glColor3f(0.5f, 0.5f, 1.0f);						// 颜色改为蓝色
	...
	glColor3f(0.5f, 1.0f, 0.0f);						// 颜色改成色
	glVertex3f(-1.0f, -1.0f, 1.0f);						// 四边形的左上顶点(底面)
	glColor3f(1.0f, 1.0f, 0.0f);						// 颜色改成黄色
	glVertex3f(-1.0f, -1.0f, -1.0f);					// 四边形的左下顶点(底面)
	glColor3f(0.0f, 0.5f, 0.5f);						// 颜色改成未知色
