...
res\Toolbar.bmp
    ...
    初始工具列和狀態列會在 CMainFrame 類別中建構。請使用資源編輯器編輯這個工具列點陣圖，並更新 DrawingTest.rc 中的 IDR_MAINFRAME TOOLBAR 陣列，以加入工具列按鈕。
/////////////////////////////////////////////////////////////////////////////
...
DrawingTestDoc.h、DrawingTestDoc.cpp - 文件
    這些檔案包含您的 CDrawingTestDoc 類別。編輯這些檔案，可加入您的特殊文件類型並實作檔案儲存和載入 (透過 CDrawingTestDoc::Serialize)。

DrawingTestView.h、DrawingTestView.cpp - 文件的檢視表
    這些檔案包含您的 CDrawingTestView 類別。
    CDrawingTestView 物件是用來檢視 CDrawingTestDoc 物件。

res\DrawingTestDoc.ico
    這是用來做為 CDrawingTestDoc 類別之 MDI 子視窗圖示的圖示檔。這個圖示是包含在主要資源檔 DrawingTest.rc 中。
...
StdAfx.h, StdAfx.cpp
    這些檔案是用來建置名為 DrawingTest.pch 的先行編譯標頭 (PCH) 檔，以及名為 StdAfx.obj 的先行編譯型別檔。

Resource.h
    這是標準標頭檔，它定義新的資源 ID。Microsoft Visual C++ 會讀取和更新這個檔案。

DrawingTest.manifest
