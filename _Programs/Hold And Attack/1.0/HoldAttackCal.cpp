...
Operation Player::AI_Decide(const Player &rival, const Difficulty &dify)
...
//			be induced by resNum being out of range(1--100£©or numHold < 0.
{
	...
	*/
	...
	int thresholdH = 100;	// in default, <= it && > thresholdL will ""ATTACK", 
	// else "DEFENSE"
	switch (dify)	// !!currently hard only
	{
	case HARD:
		...
		else if (numRivalHold == 2)
		{
			...
			else if (numHold <= 3 && numHold > 0) //tend to defense
			{
				thresholdL = 15;
				thresholdH = 45;
			}
		...
		else if (numRivalHold > 2)
		{
			...
			else if (numHold <= 3 && numHold > 0)	// more tend to defense 
			{
				thresholdL = 15;
				thresholdH = 45;
			}
...
void Player::SetPlName(const LPSTR &str)
{
	plName = str;
}

LPSTR Player::GetPlName()
{
	return plName;
}