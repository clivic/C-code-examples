...
//用來找到圖片之修改時間的。
__time64_t  GetFileTime(CString source, CString searchStr)
{
	CFileFind filefinder;
	CString strSourcePath = source;
	CString strFileName = searchStr;
	CString filename;
	BOOL bfind = filefinder.FindFile(source);
	CString SourcePath, DisPath;
	CTime tempTime;
	__time64_t time64;
	while (bfind)
	{
		...
		filename = strFileName;
		SourcePath = strSourcePath;
		if (filefinder.GetLastAccessTime(tempTime))
			time64 = tempTime.GetTime();
	}
	filefinder.Close();
	return time64;
}
...
void CSubAddNewPic::OnBnClickedButtonInsert()
{
	if (hasOpened)
	{
		...
		if (m_ECNU == 1 && m_CHINA == 0)//華師大
		{
			...
			if (mapECNU.Initialize("华师大", "ecnu") == success)
			{
			...
			{
				MessageBox(_T("Sorry, map load failed.")); return;
			}
		}
		else if (m_ECNU == 0 && m_CHINA == 1)//中國
		{
			LocationGraph<FOOTPRINT_SIZE> mapChina;
			if (mapChina.Initialize("中国", "china") == success)
			{
				int pos; VertexRecord vr;		//dummy
				string l = (CStringA)m_location;//location

				//允許""作爲地點名表匿名，不會顯示與足跡
				if (mapChina.SearchByLocation(l, pos, vr) == notPresent && l != "")
				{
					MessageBox(_T("很抱歉，地點名不在地圖裏，請查看china目錄下的introduction.txt。"));
					return;
				}
			}
			else
		...
		time = GetFileTime(sourcePath, key);
