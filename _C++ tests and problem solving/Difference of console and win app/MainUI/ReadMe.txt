...
應用程式精靈已經為您建立了這個 MainUI 應用程式。這個應用程式不僅示範了使用 MFC 的基本概念，也是您撰寫應用程式的起點。

這個檔案包含一份摘要，簡要說明構成 MainUI 應用程式的
所有檔案，它們個別的內容。
...
MainUI.h
    這是應用程式的主要標頭檔。
    它包含了特定標頭 (包括 Resource.h)，並宣告 CMainUIApp 應用程式類別。

MainUI.cpp
    這是含有應用程式類別 CMainUIApp 的主要應用程式原始程式檔。

MainUI.rc
    這是程式所用的所有 Microsoft Windows 資源的列表。它含有儲存在 RES 子目錄中的圖示、點陣圖和游標。您可以直接在 Microsoft Visual C++ 內編輯這個檔案。您的專案資源是在 1028。

res\MainUI.ico
    這是用來做為應用程式圖示的圖示檔。這個圖示是包含在主要資源檔 MainUI.rc 中。
...
MainUIDlg.h、MainUIDlg.cpp - 對話方塊
    這些檔案包含您的 CMainUIDlg 類別。這個類別會定義應用程式主對話方塊的行為。對話方塊的範本是在可於 Microsoft Visual C++ 編輯的 MainUI.rc 中。

DlgProxy.h、DlgProxy.cpp - Automation 物件
    這些檔案包含您的 CMainUIDlgAutoProxy 類別。這個類別是對話方塊的 Automation Proxy 類別，因為它會處理如何公開 Automation 控制程式可用來存取對話方塊的 Automation 方法和屬性。這些方法和屬性並非直接從對話方塊類別公開，因為在強制回應對話方塊的 MFC 應用程式中，能更加完整且簡單地將 Automation 物件與使用者介面區隔開來。
...
StdAfx.h, StdAfx.cpp
    這些檔案是用來建置名為 MainUI.pch 的先行編譯標頭 (PCH) 檔，以及名為 StdAfx.obj 的先行編譯型別檔。
